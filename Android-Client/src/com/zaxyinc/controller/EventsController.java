//	EventsController.java - handles sending and receiving client messages 
//
...
import com.zaxyinc.model.Events;
import com.zaxyinc.model.PostImages;
import com.zaxyinc.model.Posts;

public class EventsController {
	private final String LOGC = "EventsController";
	// API url:
	...
	private final String USERGRID_API_URL = "http://api.usergrid.com";
	...
	private final String USERGRID_APP = "zaxyinc/eventdrive";
	...
	private final Events drivEvents = new Events();
	...
	public void getLatestEvents() {
		...
		// client call to get all latest drivEvents
		ApiResponse resp = null;
		...
		if (resp != null && resp.getFirstEntity() != null) {
			...
			for (int i = resp.getEntityCount() - 1; i >= 0; i--) {
				...
				// grab properties from post
				Map<String, JsonNode> eventEntity = resp.getEntities().get(i)
						.getProperties();

				
				
				// add event to Events object
				drivEvents.addEvent(eventEntity);
	...
	public ApiResponse attendEvent(String eventPath) {
		
		ApiResponse response = null;

		// attempt to subscribe
		try {
			response = client.apiRequest(HttpMethod.POST, null, null, USERGRID_APP,
					"users", username, "attends", eventPath);
		} catch (Exception e) {
			response = null;
		}
		...
		// if response shows success, store account info
		if ((response != null) && !"invalid_grant".equals(response.getError())) {
			//no special Client related work
		}
	...
	public ApiResponse unattendEvent(String eventPath) {
		
		ApiResponse response = null;

		// attempt to subscribe
		try {
			response = client.apiRequest(HttpMethod.DELETE, null, null, USERGRID_APP,
					"users", username, "attends", eventPath);
		} catch (Exception e) {
			response = null;
		}
	...
	public ApiResponse createEvent(String message) {
		ApiResponse response = null;
		...
		try {
			...
			data.put("date","05/09/2012");
			data.put("time","20:45");
			data.put("name",message);
			data.put("scope","free");
			
			response = client.apiRequest(HttpMethod.POST, data , null, USERGRID_APP,
					"drivevens");
		} catch (Exception e) {
			response = null;
		}
